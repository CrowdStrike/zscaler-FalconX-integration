...
class Integration():
    ...
    def integrate(self, killswitch):
        ...
        #Ignore these lines - deprecated, but saving them here just in case
        #print("Removing redundant indicators")
        #compare = Compare()
        #new_intel = compare.new_intel_remove_duplicates()
        #zs_ingestable_cs_deleted_indicators = zs_lookup.url_look_up(zs_lookup_formatted_deleted_indicators)
        #zs_lookup_formatted_deleted_indicators = cs_intel.get_deleted_lookup_ready_urls()
        #cs_indicators = cs_intel.get_lookup_ready_urls()       #returns indicators parsed for ZScaler URL look up
