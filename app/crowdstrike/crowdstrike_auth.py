...
class CrowdStrikeAuth():
    def __init__(self):
        ...
        self.data = 'client_id=%s&client_secret=%s' %(self.clientID, self.secret)
    ...
    def refresh_token(self):
        ...
        try:
            response =  requests.post(url=self.auth_URL, data=self.data, headers=self.headers)
            ...
            if 'trace_id' in list(r['meta'].keys()):
                ...
                failure_message = self.token_failure_trace(response_code, error, trace_id)
        ...
        if response_code.startswith('20'):
            if 'access_token' in list(r.keys()):
                ...
                message = 'success'
                self.token = result
                self.logger.info("CrowdStrike Authentication: Successfully authenticated.")
