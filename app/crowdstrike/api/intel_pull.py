...
class IntelPull():
    ...
    def get_indicators(self):
        self.logger.info("Querying CrowdStrike Intel Indicators API for new indicators.")
        if self.next_page_url:
            self.holding = self.next_page_url
            self.logger.info("CrowdStrike Intel Indicators API: next_page_url found - continuing paginated requests.")
            response = requests.request("GET", self.next_page_url, headers=self.headers, data=self.payload)
        else:
            self.logger.info("CrowdStrike Intel Indicators API: next_page_url not found - initiating paginated requests.")
            response = requests.request("GET", self.indicators_url, headers=self.headers, data=self.payload)
        ...
        if 'next-page' in response_headers:
            self.next_page_url = self.base_url + response_headers['next-page'][1]
        else:
            self.next_page_url = ""
    ...
    def get_deleted_indicators(self):
        self.logger.info("Querying CrowdStrike Intel Indicators API for deleted indicators.")
        if self.deleted_next_page_url:
            self.holding_deleted = self.deleted_next_page_url
            self.logger.info("CrowdStrike Intel Indicators API: next_page_url found for deleted indicators - continuing paginated requests.")
            response = requests.request("GET", self.deleted_next_page_url, headers=self.headers, data=self.payload)
        else:
            self.logger.info("CrowdStrike Intel Indicators API: next_page_url not found for deleted indicators - initiating paginated requests.")
            response = requests.request("GET", self.deleted_indicators_url, headers=self.headers, data=self.payload)
        ...
        if 'next-page' in response_headers:
            self.deleted_next_page_url = self.base_url + response_headers['next-page'][1]
        else:
